...
class PostgresService:
    ...
    async def update_database_info(self, df: pd.DataFrame, database_name: str) -> None:
    ...
    async def update_table_info(self, df: pd.DataFrame, table_name: str) -> None:
    ...
    async def update_sample_data(self, df: pd.DataFrame, table_name: str) -> None:
        """Update sample data in PostgreSQL"""
        try:
            # Create or replace the sample data table
            async with self.engine.begin() as conn:
                await conn.execute(text(f"DROP TABLE IF EXISTS sample_{table_name}"))
                df.to_sql(f"sample_{table_name}", conn, if_exists='replace', index=False)
        except Exception as e:
    ...
    async def list_databases(self) -> List[str]:
        ...
        try:
            async with self.engine.connect() as conn:
                result = await conn.execute(text("SELECT DISTINCT database_name FROM database_metadata"))
    ...
    async def get_table_details(self, database_name: str, table_name: str) -> List[Dict[str, Any]]:
    ...
    async def update_table_details(self, df: pd.DataFrame, database_name: str, table_name: str) -> None:
